...
INPUT_NAME() {
  ...
  then
    ...
    then
      ...
      GAME_PLAYED=$(echo $($PSQL "SELECT games_played FROM users WHERE user_id=$USER_ID;") | sed 's/ //g')
...
SAVE_USER() {
  ...
  then
    ...
    CURRENT_BEST_GAME=$($PSQL "SELECT best_game FROM users WHERE username='$USER_NAME';")
    if [[ -z $CURRENT_BEST_GAME || $GUESS_COUNT -lt $CURRENT_BEST_GAME ]]
    then
      UPDATE_BEST_GAME=$($PSQL "UPDATE users SET best_game=$GUESS_COUNT WHERE username='$USER_NAME';")
    fi
  fi
